...
print_netstat_udp(struct netstat_data *ndata, u32_t *entry_buf_offset)
{
  ...
  if (udp_pcbs != NULL) {
    ...
    for (upcb = udp_pcbs; upcb != NULL; upcb = upcb->next) {
      ...
      recv_qlen = 0;
#if LWIP_IPV6
      send_qlen = IP_IS_V6(&upcb->local_ip) ? netstat_udp_sendq6(upcb) : ((upcb->recv != NULL) ? \
                                                                          netstat_udp_sendq(upcb) : 0);
#else
      send_qlen = (upcb->recv != NULL) ? netstat_udp_sendq(upcb) : 0;
#endif
      if (entry_buf_len <= 1) {
        return ERR_VAL;
      }
  ...
  for (i = 0; i < NUM_TCP_PCB_LISTS; i++) {
    ...
    for (tpcb = pcblist; tpcb != NULL; tpcb = tpcb->next) {
      ...
      if (pcblist == tcp_bound_pcbs) {
        ...
        send_qlen = (tpcb->state == SYN_RCVD) ? 0 : netstat_tcp_sendq(tpcb);
      }
