  ...
  if ((argc == 1) && (strcmp(argv[0], "-a") == 0)) {
    for (i = 0; i < DNS_MAX_SERVERS; i++) {
      ...
      if (err == ERR_OK) {
        ...
        (void)ipaddr_ntoa_r(&dns, buf, IPADDR_STRLEN_MAX);
#ifdef CUSTOM_AT_COMMAND
        (void)hi_at_printf("dns %d: %s"CRLF, i + 1, buf);
#else
...
dhcp_clients_info_show(struct netif *netif_p)
{
  ...
  (void)hi_at_printf("\tserver_id : %s"CRLF, buf);
  (void)ip4addr_ntoa_r(&(dhcp->offered_sn_mask), buf, IP4ADDR_STRLEN_MAX);
  (void)hi_at_printf("\tmask : %s, %hhu"CRLF, buf, dhcp->subnet_mask_given);
  (void)ip4addr_ntoa_r(&(dhcp->offered_gw_addr), buf, IP4ADDR_STRLEN_MAX);
  (void)hi_at_printf("\tgw : %s"CRLF, buf);
...
#ifdef LWIP_DEBUG_OPEN
    (void)hi_at_printf("%s%u"CRLF, netif_p->name, netif_p->num);
#else
    LWIP_PLATFORM_PRINT("%s%u"CRLF, netif_p->name, netif_p->num);
#endif
...
#ifdef LWIP_DEBUG_OPEN
      (void)hi_at_printf("\t%-50s%-16hhu%-16hhu%-16hu%-16hhu"CRLF, buf,
...
#ifdef LWIP_DEBUG_OPEN
      (void)hi_at_printf("\t%-50s%-16hhu%-16hhu%-16hu%-16hu"CRLF, buf,
...
#ifdef LWIP_DEBUG_OPEN
      (void)hi_at_printf("\t%-16s%-16hhu%-16d%-16hhu%-16d"CRLF, buf,
                         igmp_group_p->last_reporter_flag, igmp_group_p->group_state,
                         igmp_group_p->timer, igmp_group_p->use);
#else
      LWIP_PLATFORM_PRINT("\t%-16s%-16hhu%-16d%-16hhu%-16d"CRLF, buf,
                          igmp_group_p->last_reporter_flag, igmp_group_p->group_state,
                          igmp_group_p->timer, igmp_group_p->use);
#endif
...
dhcps_info_show(struct netif *netif)
{
...
#ifdef CUSTOM_AT_COMMAND
  (void)hi_at_printf("start ip : %s"CRLF, buf);
#else
  ...
  (void)ip4addr_ntoa_r(&addr, buf, IP4ADDR_STRLEN_MAX);
#ifdef CUSTOM_AT_COMMAND
  (void)hi_at_printf("end ip : %s"CRLF, buf);
  ...
  for (i = 0; i < dhcps->lease_num; i++) {
    if (dhcps->leasearr[i].flags != 0) {
      ...
      (void)ip4addr_ntoa_r(&addr, buf, IP4ADDR_STRLEN_MAX);
#ifdef CUSTOM_AT_COMMAND
      (void)hi_at_printf(" %s leasetime = %u(%u), proposed_leasetime = %u"CRLF, buf, lease->leasetime,
