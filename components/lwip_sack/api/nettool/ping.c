...
lwip_ping_usage(u32_t is_v6)
{
  ...
  LWIP_PLATFORM_PRINT(CRLF"  -t means ping forever, user can use -k to stop the forever ping");
#else
  (void)is_v6;
#endif
}
...
#define PING_ADDR_STR_LEN IP6ADDR_STRLEN_MAX
...
#else
    LWIP_PLATFORM_PRINT("Ping: pbuf_resp malloc failed"CRLF);
#endif
    return -1;
  }
...
#else
    LWIP_PLATFORM_PRINT("Ping: echo request malloc failed"CRLF);
#endif
    return -1;
  }
  ctx->to = cfg->dst;
...
#else
  LWIP_PLATFORM_PRINT("Ping: %s ..."CRLF, print_str);
#endif
}
...
ping_rcv(struct ping_run_ctx *ctx)
{
  ...
  do {
    ...
    if (ret < 0) {
#ifdef CUSTOM_AT_COMMAND
      (void)hi_at_printf("ping : poll/select failure, errno = %d"CRLF, errno);
#else
    ...
    if (ret < 0) {
#ifdef CUSTOM_AT_COMMAND
      (void)hi_at_printf(CRLF"Recv failed errno = %d..."CRLF, errno);
#else
...
LWIP_STATIC int ping_setsockopt(struct ping_run_ctx *ctx, struct ping_cfg *cfg)
{
  ...
  if (cfg->src_type == LWIP_PING_HOSTNAME_ARG) {
    ...
    if (ret == -1) {
      ...
      (void)hi_at_printf("ping: bind icmp socket: cannot assign requested address, errno = %d"CRLF, errno);
#else
      LWIP_PLATFORM_PRINT("ping: bind icmp socket: cannot assign requested address, errno = %d"CRLF, errno);
#endif
      return -1;
    }
...
os_ping_func(struct ping_cfg *cfg)
{
  ...
  if (ctx.sfd < 0) {
#ifdef CUSTOM_AT_COMMAND
    (void)hi_at_printf("Ping socket create fail, errno = %d"CRLF, errno);
#else
    perror("Ping socket");
#endif
    mem_free(cfg);
...
#else
      LWIP_PLATFORM_PRINT("Ping: sending ICMP echo request failed errno %d"CRLF, errno);
#endif
      goto FAILURE;
    }
...
ping_cmd(unsigned int p0, unsigned int p1, unsigned int p2, unsigned int p3)
{
  ...
  ptr_args.args[0] = p0;
  ptr_args.args[1] = p1;
  ret = os_ping_func((struct ping_cfg *)ptr_args.ptr);
  if (ret < 0) {
#ifdef CUSTOM_AT_COMMAND
    (void)hi_at_printf(CRLF"ERROR"CRLF);
#else
    LWIP_PLATFORM_PRINT(CRLF"ERROR"CRLF);
#endif
  }
...
#else
  LWIP_PLATFORM_PRINT("ping : Host : %s can't be resolved to IP address"CRLF, daddr);
#endif
  return -1;
}
...
#else
            LWIP_PLATFORM_PRINT("Ping %s arg error, should be in range of [%d, %d] "CRLF,
                   handle[j].key, handle[j].min, handle[j].max);
#endif
            return -1;
          }
...
#else
    LWIP_PLATFORM_PRINT("Invalid Ping param"CRLF);
#endif
    return -1;
  }
...
#else
    LWIP_PLATFORM_PRINT("Ping task already running and only support one now"CRLF);
#endif
    return LOS_NOK;
  }
  ptr_args.ptr = cfg;
...
#else
    LWIP_PLATFORM_PRINT("ping_task create failed 0x%08x."CRLF, los_ret);
#endif
    return LOS_NOK;
  }
  return LOS_OK;
...
LWIP_STATIC u32_t ping_exec(struct ping_cfg *cfg)
{
  ...
  if ((cfg->count == 0) || (cfg->count > LWIP_SHELL_CMD_PING_RETRY_TIMES)) {
    ...
    if ((ret == LOS_NOK) || (ping_taskid == 0)) {
      LWIP_PLATFORM_PRINT("ping_task create failed"CRLF);
      cfg->count = LWIP_SHELL_CMD_PING_RETRY_TIMES;
    } else {
      return LOS_OK;
    }
  ...
  if (os_ping_func(cfg) < 0) {
    LWIP_PLATFORM_PRINT("Ping cmd failed due some errors"CRLF);
    return LOS_NOK;
  }
...
#else
    LWIP_PLATFORM_PRINT("%s: mem_malloc faild"CRLF, __FUNCTION__);
#endif
    return LOS_NOK;
  }
  cfg->count = 0;
...
#ifdef CUSTOM_AT_COMMAND
      (void)hi_at_printf("+PING%s:"CRLF"OK"CRLF, is_v6 ? "6" : "");
#else
...
#else
      LWIP_PLATFORM_PRINT("%s: mem_malloc failed"CRLF, __FUNCTION__);
#endif
      return LOS_NOK;
    }
