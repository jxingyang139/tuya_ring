...
/* when rpl route is changed, we should send event to meshautolink module
 * 0: meshautolink call rpl api,we don't send event
 * 1: rpl route is changed by itself, we send event to meshautolink
 */
#define RPL_ROUTE_NO_SEND_EVENT 0
#define RPL_ROUTE_SEND_EVENT 1
...
void rpl_route_delete_by_address(const rpl_addr_t *addr, uint8_t len, const rpl_addr_t *nhop);
...
uint8_t rpl_route_init();
void rpl_route_deinit();
uint8_t rpl_netif_get_link_metric(uint16_t *metric);
uint8_t ripple_route_entry_delete(rpl_rte_t *target);
uint8_t ripple_route_entry_sync_delete_by_mnid(uint8_t mnid);
uint8_t ripple_route_entry_add(rpl_rte_t *target);
uint8_t ripple_route_entry_update(rpl_rte_t *target, uint8_t type);
void rpl_send_route_update_ind(inst_t inst_id, rpl_rte_t *rte, uint8_t type);
