...
#define LPOP_INDEX_COUNT (LPOP_CIRCLE_MAX + LPOP_SEQ_WIN)
...
#define LPOP_INCR_EXCEPT_INIT(val) do { \
  if (val != LPOP_INIT) {  \
    LPOP_INCR(val);        \
  }                        \
} while (0)
...
uint8_t rpl_mmbr_mnid_init(void);
uint8_t rpl_mmbr_mnid_alloc(rpl_linkaddr_t *lladdr, rpl_mnode_id_t prfid, rpl_mnode_id_t *mnid);
void rpl_mmbr_mnid_release(rpl_linkaddr_t *lladdr, rpl_mnode_id_t mnid);
#endif
