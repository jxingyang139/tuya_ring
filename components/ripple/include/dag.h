...
#define DAO_ACK_GET_MNID_FAIL_THRESHOLD 10
...
typedef struct {
  ...
  uint8_t daoack_get_mnid_fail_cnt;
#endif
...
void rpl_mbr_start_with_valid_mnid(rpl_dag_t *dag);
...
uint8_t rpl_instance_init();
void rpl_instance_deinit();
void rpl_target_msta_delete_all(rpl_dag_t *dag);
void rpl_target_check(void);
uint8_t rpl_target_count(rpl_dag_t *dag);
#endif /* _RPL_DAG_H_ */
