...
typedef struct rpl_peer_s {
  struct rpl_peer_s *next;
  rpl_linkaddr_t peer_mac;
  uint32_t conn_time;
  uint8_t is_mesh;
} rpl_peer_t;

typedef struct rpl_ipv4_rqst_s {
  const rpl_addr_t *ip6addr;
  const rpl_linkaddr_t *lladdr;
  uint32_t lifetime;
  uint32_t *ipv4;
  uint32_t conn_time;
  uint8_t dao_sn;
 ...
 *
 * @param rte_info - output next Route table entires, can be NULL.
 *
 * @return RPL_OK if route entry found, RPL_FAIL if not found.
 */
uint8_t rpl_platform_get_next_rte(rpl_rte_info_t *rte_info);
 ...
 * @return OK/FAIL
 */
uint8_t rpl_core_get_dag_details(inst_t *inst_id, uint16_t *rank, rpl_addr_t *dag_id,
                                 uint8_t pkt_up, const rpl_addr_t *src_addr);
...
uint32_t *rpl_request_ipv4addr(const rpl_ipv4_rqst_t *ipv4_rqst);
...
void rpl_peer_conn_time_update(void);
void rpl_peer_conn_time_clear(void);
uint8_t rpl_peer_lookup(const uint8_t *mac, uint8_t mac_len, uint8_t is_mesh, void **out_peer);
