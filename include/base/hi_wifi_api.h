 ...
 */
typedef int (*hi_wifi_rx_mgmt_cb)(void* recv_buf, int frame_len, signed char rssi);
...
/**
* @ingroup  hi_wifi_basic
* @brief  Set scan.strategy.CNcomment:设置扫描策略.CNend
*
* @par Description:
*           Set scan_strategy.cnt,support the largest 10.CNcomment:设置扫描策略,支持最大设置slot数为10.CNend
*
* @attention  1.Set slot before starting the scan.
*             CNcomment:扫描的slot数需在扫描之前设置。
*             2.Restart can restore default slot,
*             CNcomment:关闭STA/AP命令再启动STA/AP可以恢复默认值。
*             3.Valid slot:1~10.
*             CNcomment:有效扫描slot数为1~10.
*             4.Set the number of scan slots, one slot is 30ms.
*             CNcomment:设置扫描slot数，一个slot 30ms。
*             5.Default slot is 2.
*             CNcomment:默认扫描slot数为2.
*
* @param  ifname           [IN]     Type  #const char *, interface name.CNcomment:接口名.CNend
* @param  scan_strategy    [IN]     Type  #hi_wifi_scan_strategy_stru *scan_strategy:扫描策略结构体.CNend
*
* @retval #HISI_OK  Excute successfully
* @retval #Other           Error code
* @par Dependency:
...
* @see  NULL
*/
int hi_wifi_set_scan_strategy(const char *ifname, hi_wifi_scan_strategy_stru *scan_strategy);
...
*
* @attention 1.need to invoke hi_wifi_config_report_mgmt(NULL) before enable low power mode.
*             CNcomment:开启低功耗睡眠之前需要调用hi_wifi_config_report_mgmt(NULL)关闭管理帧上报，不然会引起功耗偏高。
* @param  data_cb          [IN]     Type  #hi_wifi_rx_mgmt_cb callback function pointer.CNcomment:管理帧上报回调函数.CNend
*
* @retval #HI_ERR_SUCCESS  Excute successfully
* @retval #Other           Error code
* @par Dependency:
...
* @see  NULL
*/
int hi_wifi_config_report_mgmt(hi_wifi_rx_mgmt_cb data_cb);
...
/**
* @ingroup  hi_wifi_basic
* @brief  Set low boot current mode.CNcomment:设置低启动电流模式.CNend
*
* @par Description:
*           Set low boot current mode.CNcomment:设置低启动电流模式.CNend
*
* @attention  1.Should be called before hi_wifi_init().
*               CNcomment:只能在hi_wifi_init()函数前调用.CNend
*             2.Mask power calibration will make tx power inaccuracy.
*              CNcomment:屏蔽功率校准会导致发送功率不准确.CNend
*             3.Mask all calibration will degrade RF ability.
*              CNcomment:屏蔽所有上电校准项会导致RF性能下降.CNend
* @param  flag        [IN]     Type #unsigned char, mode setting.1:mask power calibration,<300mA.0xff:mask all calibration.
*                                 CNcomment:模式设置,1:屏蔽功率校准,启动电流不大于300mA; 2:只屏蔽非首次上电的功率校准;
*                                           0xff:屏蔽所有上电校准项.CNend
* @retval #HISI_OK  Excute successfully
* @retval #Other           Error code
*
* @par Dependency:
...
* @see  NULL
*/
unsigned int hi_wifi_enable_low_boot_current(unsigned char flag);

/**
* @ingroup  hi_wifi_basic
* @brief  Set rts send-mode according to packet length.CNcomment:设置RTS发送模式.CNend
*
* @par Description:
*           Set rts send-mode according to packet length.CNcomment:设置RTS发送模式.CNend
*
* @attention  1. Once invoked, this API will be effective for both Station and SoftAp;
*             CNcomment:
              1. 该API为全局配置，对于STA或者AP都生效;
              CNend
* @param  mode         [IN]     Type #unsigned char, 0:automatic, 1:send rts according to packet length, 2:don't send rts.
*                                 CNcomment:RTS发送模式:
*                                 0:自动模式（默认）
*                                 1:超过配置报文长度发送RTS,未超过按照自动模式发或者不发
*                                 2:不发RTS CNend
* @param  pkt_length   [IN]     Type #unsigned short, packet length as RTS threshold.
*                                 CNcomment:模式为1时作为RTS报文长度门限，报文长度包括MAC头长度.CNend
*
* @retval #HISI_OK  Excute successfully
* @retval #Other           Error code
*
...
* @see  NULL
*/
int hi_wifi_set_rts_threshold(unsigned char mode, unsigned short pkt_length);
...
/**
* @ingroup  hi_wifi_basic
* @brief  Set linkloss parameters.CNcomment:设置linkloss相关参数.CNend
*
* @par Description:
*           Set linkloss parameters.CNcomment:设置linkloss相关参数.CNend
*
* @param    linkloss_paras    [IN]    Type #hi_linkloss_paras_stru *, linkloss parameters.CNcomment:设置linkloss相关参数.CNend
*
* @retval #HISI_OK  Excute successfully
* @retval #Other           Error code
*
...
* @see  NULL
*/
int hi_wifi_set_linkloss_paras(hi_linkloss_paras_stru *linkloss_paras);
