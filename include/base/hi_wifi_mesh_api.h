...
typedef struct {
    ...
    unsigned char mesh_trigger;             /* mesh trigger标志 */
 ...
 */
typedef enum {
    HISI_MESH_UNSPEC = 0, /* 未确定mesh节点角色 */
    HISI_MESH_STA,        /* Mesh-STA节点角色 */
    HISI_MESH_MG,         /* Mesh-MG节点角色 */
    HISI_MESH_MBR,        /* Mesh-MBR节点角色 */

    HISI_MESH_NODE_BUTT,
} hisi_mesh_node_type_enum;
typedef unsigned char hisi_mesh_node_type_enum_uint8;
 ...
 */
typedef struct {
    hisi_mesh_node_type_enum_uint8 node_type;               /* 本节点角色 */
...
*             4. The length of key is limited from 8 to 63.CNcomment:4．key密码长度仅支持8-63位.CNend
...
*             5.Only mesh and mesh_sta that are successfully associated are displayed. \n
*             CNcomment:5.只显示关联成功的mesh和mesh_sta。CNend
...
*
* @attention  NULL
* @param  mesh_node_info     [OUT]     Type  #hi_mesh_nodeinfo_stru *, node info.CNcomment:节点信息.CNend
*
* @retval #HISI_OK  Excute successfully
* @retval #Other           Error code
* @par Dependency:
...
* @since Hi3861_V100R001C00
*/
unsigned int hi_wifi_get_mesh_node_info(hi_mesh_nodeinfo_stru *mesh_node_info);

/**
* @ingroup  hi_wifi_mesh
* @brief  Set mesh trigger config network switch.CNcomment:设置触发mesh配网开关.CNend
*
* @par Description:
*         Set mesh trigger config network switch.CNcomment:设置触发mesh配网开关.CNend
*
* @attention  1.MESH AP set enable to send trigger message.CNcomment:1.MESH AP 设置此接口以发送trigger信息.CNend \n
*             2.MESH STA set enable to receive trigger message.CNcomment:2.MESH STA 设置此接口以接收trigger信息.CNend \n
*             3.Should be called after MESH AP or MESH STA started.CNcomment:3.MESH STA或 MESH AP启用后调用此接口.CNend \n
*
* @param enable_trigger    [IN]     Type  #unsigned char, enable switch, value：0 or 1.CNcomment:使能开关,值为0或1.CNend
*
* @retval #HISI_OK  Excute successfully
* @retval #Other           Error code
* @par Dependency:
