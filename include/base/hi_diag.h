...
typedef hi_u32 (*hi_diag_offline_log_output_hook)(const hi_u8 *data, hi_u32 data_len, hi_u32 level);

/**
* @ingroup  iot_diag
* @brief  Set the log output level. CNcomment:设置日志的输出级别。CNend
*
* @par 描述:
*      Set the log output level. If the output level is set to DIAG_LEVEL_ERROR, only ERROR logs can be output.
If the output level is set to DIAG_LEVEL_WARN, ERROR and WARNING logs can be output.
CNcomment:设置日志的输出级别，如果输出级别设置为DIAG_LEVEL_ERROR，只有ERROR日志可以输出，如果输出级别设置为DIAG_LEVEL_WARN，
ERROR和WARNING的日志可以输出。
*
* @attention None
* @param  log_level      [IN] type #diag_msg_level, log output level. CNcomment:日志输出级别。CNend
CNcomment:日志输出级别，取值从:DIAG_LEVEL_ERROR - DIAG_LEVEL_INFO。CNend
*
* @retval None
* @par 依赖:
*           @li hi_diag.h：Describes DIAG APIs.
...
* @see  None
*/
hi_void hi_diag_set_log_output_level(diag_msg_level log_level);

/**
* @ingroup  iot_diag
* @brief  Register diag offline log output function to output offline log to user.
CNcomment:注册DIAG离线日志输出函数，以便将DIAG离线日志输出给用户。CNend
*
* @par 描述:
*      Register diag offline log output function to output offline log to user. Users can save offline logs and
import them to the HSO for parsing.
CNcomment:注册DIAG离线日志输出函数，以便将DIAG离线日志输出给用户。用户可以将离线日志保存下来，再导入到HSO中解析。CNend
*
* @attention Should set suitable task size according to implementation of output func.
CNcomment:根据输出函数实现，设置合适的栈大小。CNend
* @param  func      [IN] type #hi_diag_offline_log_output_hook，offline log output function.
CNcomment:DIAG离线日志输出函数。CNend
*
* @retval None
* @par 依赖:
*           @li hi_diag.h：Describes DIAG APIs.
CNcomment:文件用于描述DIAG相关接口。CNend
*/
hi_void hi_diag_register_offline_log_output_hook(hi_diag_offline_log_output_hook func);
