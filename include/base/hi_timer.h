...
*
* @attention This timer callback function is executed in the Swt_Task, which has the highest priority. Therefore, the callback
*            function should be as simple as possible and the functions such as sleep and wait semaphores that trigger
*            scheduling cannot be used.CNcomment:本定时器回调函数执行于最高优先级的Swt_Task任务中，回调函数应尽量简单，不能使用睡眠、
等待信号量等引起调度的函数，避免阻塞其它定时器响应。CNend
*
