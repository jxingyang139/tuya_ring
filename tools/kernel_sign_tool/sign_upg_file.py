...
import sign_user_info as USER_INFO
...
def print_upg_info(image_file, upg_bin):
    ...
    print("---------sizeof(hi_upg_common_head) = %d - sizeof(hi_upg_section_head) = %d--------------->"%(sizeof(hi_upg_common_head), sizeof(hi_upg_section_head)))
    #打印common段key信息、common段签名信息、section段签名信息
    print("------st_hupg.param.sign_alg = %d------"%(st_hupg.param.sign_alg))
...
def make_hbin(flash_boot_file, factory_nv_file, normal_nv_file, upg_file, image_file):
    ...
    print('hbin normal nv file  in :', normal_nv_file)
    print('hbin upg file        in :', upg_file)
