...
#define VLINK_UPGRADE_TASK_STAK_SIZE (1024*20)
...
static int http_download(int socket_d, uintptr_t kernel_upg_addr)
{
    ...
    {
        //write(fd, buf, len);
	printf("=================start=write addr[0x%X]======len[0x%X]===============\n", kernel_upg_addr + length, len);
...
static int get_http_response_data(uintptr_t kernel_upg_addr, hi_u8 kernel_index)
{
	...
	char url[100] = "http://192.168.3.116:8081/ubuntu/bin/Hi3861L_demo_ota.bin";
	...
	if (kernel_index == 1)
	{
		sprintf(header,"GET %s HTTP/1.1\r\nHost: %s\r\n", "/ubuntu/bin/Hi3861L_demo_ota.bin", domain); 
		printf("download FILENAME: %s\n\n", "Hi3861L_demo_ota.bin");
...
static int vlink_test_start_fota_process(hi_void)
{
	hi_u32 max_len; 
	hi_u8 file_index;
	hi_u8 kernel_index = 2;
...
#if 0
static int vlink_test_httpclient(hi_void)
{
	hi_u32 dhcpEvent = 0;
	...
	{ 
		...
		{
			MLOGD("wait event dhcp success===\n"); 
			vlink_test_start_fota_process();

		} else {
			MLOGE("wait event dhcp fail===\n"); 
		}
	} 
	else 
	{ 
		MLOGD("read event fail!===ret=0X%0X, dhcpEvent = %d--\n", ret, dhcpEvent); 
	} 	
...
hi_s32 vlink_start_fota_proc_ota_data(hi_char *sendbuff, hi_u32 length)
{
	hi_s32 ret = HI_ERR_SUCCESS;
	hi_u32 write_len = length - 3;
	...
	ret = hi_upg_transmit(g_ota_kernel_upg_addr, &sendbuff[3], write_len);
	if (ret != HI_ERR_SUCCESS) {
	...
	g_ota_kernel_upg_addr = g_ota_kernel_upg_addr + write_len;
	//hi_sleep(10); /* sleep 10s */

	vlink_fota_send_ok_msg_to_camera("0");
...
static int vlink_start_fota_process(hi_void)
{
	...
	{ 
		if (otaEvent & WIFI_OTA_SUCCESS) 
		{
			...
			//PDT_SYSTEM_SYSLINK_SendOtaSuccessOrFailMsgToCamera(1);
		} else {
			MLOGE("===wifi ota fail===\n");
			vlink_fota_send_ok_msg_to_camera("1");
			return -1; 
		}
	} 
	else 
...
hi_void vlink_start_startota_task(hi_void)
{
    ...
    dzattr.task_name = (hi_char*)"update_process_main";
    ret = hi_task_create(&g_vlink_update_task_id, &dzattr, update_process_main_task, HI_NULL);
    if (ret != HI_ERR_SUCCESS) {
        MLOGE("Falied to create vlink_upgrade_process_main task!\n");
    }
}
