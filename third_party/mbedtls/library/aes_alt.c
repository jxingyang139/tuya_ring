                    ...
                    unsigned int keybits)
{
    ...
    ctx->ccm = HI_NULL;
    if (memcpy_s(ctx->key, sizeof(ctx->key), key, (keybits/8)) != EOK) {
                                 ...
                                 bool type)
{
    ...
    cipherRet = hi_cipher_aes_config(ctx);
    if( cipherRet != HI_ERR_SUCCESS )
        return MBEDTLS_ERR_AES_HW_ACCEL_FAILED;

    cipherRet = hi_cipher_aes_crypto(alignedInBuf, alignedOutBuf, AES_ALT_BLOCK_SIZE, type);
    if( cipherRet == HI_ERR_SUCCESS )
    {
                    ...
                    unsigned char *output)
{
    ...
    cipherRet = hi_cipher_aes_crypto((uintptr_t)temp_input, (uintptr_t)temp_output, length, (mode == MBEDTLS_AES_ENCRYPT));
    if( cipherRet != HI_ERR_SUCCESS )
    {
        ret = MBEDTLS_ERR_AES_HW_ACCEL_FAILED;
        goto exit;
