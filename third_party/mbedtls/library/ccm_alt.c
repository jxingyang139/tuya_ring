...
static int set_aes_config(hi_u8 *key, hi_u32 klen,
                           ...
                           unsigned char *tag, size_t tag_len )
{
    ...
    cipherRet = hi_cipher_aes_config( &aes_ctrl );
    ...
    cipherRet = hi_cipher_aes_crypto( (uintptr_t)temp_input, (uintptr_t)temp_output, length, (mode == CCM_ENCRYPT) );
    if( cipherRet == HI_ERR_SUCCESS )
    {
    ...
    cipherRet = hi_cipher_aes_get_tag( tag, tag_len, &enc_tag_out_len );
    if( cipherRet != HI_ERR_SUCCESS)
        ret = MBEDTLS_ERR_CCM_HW_ACCEL_FAILED;
                         ...
                         unsigned char *tag, size_t tag_len )
{
    ...
    if( cipher_info->key_bitlen == 128 && cipher_info->base->cipher == MBEDTLS_CIPHER_ID_AES )
