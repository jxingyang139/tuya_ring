...
#define FLASH_ERASE_WHOLE_SIZE         0x200000
#define FLASH_ERASE_WHOLE_FLAG         0xFFFFFFFF
...
hi_u32 download_to_flash(hi_u32 flash_offset, hi_u32 erase_size, hi_u32 flash_size)
{
    ...
    if (erase_size == FLASH_ERASE_WHOLE_FLAG) { /* 0xFFFFFFFF indicates erase whole */
        boot_msg0("erase flash whole");
        ret = g_flash_cmd_funcs.erase(0, FLASH_ERASE_WHOLE_SIZE);
        if (ret != HI_ERR_SUCCESS) {
        ...
        erase_size = FLASH_ERASE_WHOLE_FLAG; /* change to 0xFFFFFFFF, in order to write flash in loader_serial_ymodem */
    }
