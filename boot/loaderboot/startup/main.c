...
hi_void stack_guard_init(hi_void)
{
    hi_ulong rand;

    hi_u32 ret = hi_cipher_trng_get_random_bytes((hi_u8*)&rand, sizeof(rand));
    if (ret != HI_ERR_SUCCESS) {
        boot_msg0("canary get random error\r\n");
    } else {
        set_stack_chk_guard(rand);
    }
}
