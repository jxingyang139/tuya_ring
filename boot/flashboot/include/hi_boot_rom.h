...
*
* @attention The value of the val can only be the enumerated value that corresponding to the parameter id.
*            For example, if id is set to HI_IO_NAME_GPIO_2, val can only be the value in hi_io_func_gpio_2.
*            CNcomment:变量val的值只支持变量id对应的枚举值。比如id设置为HI_IO_NAME_GPIO_2时,
*                      val只能使用hi_io_func_gpio_2里的值。CNend
