...
static int write_cmd(char *buf, size_t buflen, int argc, char *argv[])
{
	...
	int sum;
	pos = buf;
	if ((buflen < 1) || (argc > MAX_WPA_CLI_ARGC_NUM))
		return HISI_FAIL;
	...
	sum = res;
	for (i = 0; i < argc; i++) {
		res = snprintf_s(pos, (size_t)(buflen - sum), (size_t)(buflen - sum - 1), " %s", argv[i]);
		if (res < 0)
			goto fail;
		pos += res;
		sum += res;
...
int wpa_cli_configure_wep(struct wpa_supplicant *wpa_s, const char *id, const struct wpa_assoc_request *assoc)
{
	...
	if ((wpa_cli_set_network(wpa_s, id, "auth_alg", "OPEN SHARED") != HISI_OK) ||
	    (wpa_cli_set_network(wpa_s, id, "key_mgmt", "NONE") != HISI_OK))
		return HISI_FAIL;
	if ((snprintf_s(command, sizeof(command), sizeof(command) - 1, "wep_key%d", 0) < 0) ||
	    (wpa_cli_set_network(wpa_s, id, command, assoc->key) != HISI_OK))
		return HISI_FAIL;
	if ((snprintf_s(command, sizeof(command), sizeof(command) - 1, "%d", 0) < 0) ||
	    (wpa_cli_set_network(wpa_s, id, "wep_tx_keyidx", command) != HISI_OK))
		return HISI_FAIL;
