...
static inline void hisi_driver_event_del_sta_process(hisi_driver_data_stru *drv, const int8 *data_ptr)
{
	union wpa_event_data event;
...
static inline void hisi_driver_event_channel_switch_process(hisi_driver_data_stru *drv, const int8 *data_ptr)
{
	union wpa_event_data event;
...
static void hisi_driver_event_connect_result_process(hisi_driver_data_stru *drv, int8 *data_ptr)
{
	...
	if (accoc_info->status != 0) {
		...
		if ((accoc_info->status == WLAN_STATUS_INVALID_PMKID) ||
		    (accoc_info->status == WLAN_STATUS_ASSOC_REJECTED_TEMPORARILY) ||
...
static void hisi_driver_event_timeout_disc_process(const hisi_driver_data_stru *drv)
{
...
static inline void hisi_driver_event_scan_done_process(hisi_driver_data_stru *drv, const int8 *data_ptr)
{
...
INIT_FAILED:
	if (send_event_cb_reg_flag == HISI_SUCC)
		(void)hisi_register_send_event_cb(NULL);
	hisi_drv_deinit(drv);
	return NULL;
}
...
void hisi_hapd_deinit(void *priv)
{
	...
	if ((hisi_ioctl_set_netdev(drv->iface, &status) != HISI_SUCC) ||
	    (hisi_ioctl_set_mode(drv->iface, &set_mode) != HISI_SUCC)) {
		wpa_error_log0(MSG_ERROR, "hisi_hapd_deinit , hisi_ioctl_set fail.");
		return;
	}
...
int32 hisi_send_ext_auth_status(void *priv, struct external_auth *params)
{
	int32 ret;
	hisi_driver_data_stru *drv = priv;
	hisi_external_auth_stru *external_auth_stru = NULL;
