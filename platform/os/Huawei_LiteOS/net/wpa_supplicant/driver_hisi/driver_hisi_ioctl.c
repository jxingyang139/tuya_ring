...
{
	return hisi_ioctl_command_set(ifname, ifname, HISI_IOCTL_DISABLE_EAPOL);
}

int32 hisi_ioctl_get_own_mac(const int8 *ifname, const int8 *mac_addr)
{
	return hisi_ioctl_command_set(ifname, mac_addr, HIIS_IOCTL_GET_ADDR);
}

int32 hisi_ioctl_set_ap(const int8 *ifname, const void *buf)
{
	return hisi_ioctl_command_set(ifname, buf, HISI_IOCTL_SET_AP);
}

int32 hisi_ioctl_change_beacon(const int8 *ifname, const void *buf)
{
	return hisi_ioctl_command_set(ifname, buf, HISI_IOCTL_CHANGE_BEACON);
}
...
{
	return hisi_ioctl_command_set(ifname, buf, HISI_IOCTL_SEND_MLME);
}

int32 hisi_ioctl_new_key(const int8 *ifname, const void *buf)
{
	return hisi_ioctl_command_set(ifname, buf, HISI_IOCTL_NEW_KEY);
}

int32 hisi_ioctl_del_key(const int8 *ifname, const void *buf)
{
	return hisi_ioctl_command_set(ifname, buf, HISI_IOCTL_DEL_KEY);
}

int32 hisi_ioctl_set_key(const int8 *ifname, const void *buf)
{
	return hisi_ioctl_command_set(ifname, buf, HISI_IOCTL_SET_KEY);
}

int32 hisi_ioctl_set_mode(const int8 *ifname, const void *buf)
{
	return hisi_ioctl_command_set(ifname, buf, HISI_IOCTL_SET_MODE);
}

int32 hisi_ioctl_set_netdev(const int8 *ifname, const hisi_bool_enum_uint8 *netdev)
{
	return hisi_ioctl_command_set(ifname, netdev, HISI_IOCTL_SET_NETDEV);
}

int32 hisi_ioctl_scan(const int8 *ifname, const void *buf)
{
	return hisi_ioctl_command_set(ifname, buf, HISI_IOCTL_SCAN);
}

int32 hisi_ioctl_disconnet(const int8 *ifname, const void *buf)
{
	return hisi_ioctl_command_set(ifname, buf, HISI_IOCTL_DISCONNET);
}

int32 hisi_ioctl_assoc(const int8 *ifname, const void *buf)
{
	return hisi_ioctl_command_set(ifname, buf, HISI_IOCTL_ASSOC);
}

#ifdef _PRE_WLAN_FEATURE_REKEY_OFFLOAD
int32 hisi_ioctl_set_rekey_info(const int8 *ifname, const void *buf)
{
	return hisi_ioctl_command_set(ifname, buf, HISI_IOCTL_SET_REKEY_INFO);
}
#endif

int32 hisi_ioctl_send_action(const int8 *ifname, const void *buf)
{
	return hisi_ioctl_command_set(ifname, buf, HISI_IOCTL_SEND_ACTION);
}

int32 hisi_ioctl_mesh_usr_add(const int8 *ifname, const void *buf_addr)
{
	return hisi_ioctl_command_set(ifname, buf_addr, HISI_IOCTL_SET_MESH_USER);
}

int32 hisi_ioctl_mesh_set_mgtk(const int8 *ifname, const void *buf_addr)
{
	return hisi_ioctl_command_set(ifname, buf_addr, HISI_IOCTL_SET_MESH_GTK);
}
                                  ...
                                  const void *buf_addr)
{
	...
	return hisi_ioctl_command_set(ifname, buf_addr, (unsigned int)hisi_get_cmd_from_mesh_flag(flag_type));
}

 int32 hisi_ioctl_set_usr_app_ie(const int8 *ifname, const void *usr_app_ie)
{
	return hisi_ioctl_command_set(ifname, usr_app_ie, HISI_IOCTL_SET_USR_APP_IE);
}

int32 hisi_ioctl_sta_remove(const int8 *ifname, const void *buf_addr)
{
	return hisi_ioctl_command_set(ifname, buf_addr, HISI_IOCTL_STA_REMOVE);
}

int32 hisi_ioctl_get_drv_flags(const int8 *ifname, const void *buf_addr)
{
	return hisi_ioctl_command_set(ifname, buf_addr, HISI_IOCTL_GET_DRIVER_FLAGS);
}

int32 hisi_ioctl_set_delay_report(const int8 *ifname, const void *buf)
{
	return hisi_ioctl_command_set(ifname, buf, HISI_IOCTL_DELAY_REPORT);
}

int32 hisi_ioctl_send_ext_auth_status(const int8 *ifname, const void *buf)
{
	return hisi_ioctl_command_set(ifname, buf, HISI_IOCTL_SEND_EXT_AUTH_STATUS);
}

int32 hisi_ioctl(const int8 *ifname, const hisi_ioctl_command_stru *ioctl_cmd)
{
	int32 ret;
