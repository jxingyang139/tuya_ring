	...
	*sta_removed = 0;
	...
	case SAE_COMMITTED:
		...
		if (auth_transaction == 1) {
			 ...
			 */
#ifdef CONFIG_HOSTAPD_WPA3
			/* Due to chip performance issues, the driver receives the Assoc frame and
			   then the auth status message. As a result, the driver process is incorrect.
			   Therefore, the auth status is sent before the confirm frame to ensure that
			   the auth status reaches the driver first. */
			sae_sme_send_external_auth_status(hapd, sta, WLAN_STATUS_SUCCESS);
#endif /* CONFIG_HOSTAPD_WPA3 */
			ret = auth_sae_send_confirm(hapd, sta, bssid);
			if (ret)
