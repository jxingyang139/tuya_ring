	...
	const struct wpa_bss *bss, char *buf, size_t buflen)
{
	...
	if (hisi_mesh) {
		...
		if (((wpa_s->ifmsh != NULL) || g_mesh_sta_flag) &&
			...
			if (mesh_ap_param.mesh_trigger) {
				ret = os_snprintf(pos, end - pos, "[TRIG]");
				if (os_snprintf_error(end - pos, ret))
					return -1;
				pos += ret;
			}
...
static int wpa_mesh_deauth_peer(struct wpa_supplicant *wpa_s, const char *addr_txt)
{
	...
	if (sta->plink_state >= PLINK_IDLE) {
		if (wpas_mesh_peer_remove(wpa_s, addr) != HISI_OK) {
			wpa_error_log0(MSG_ERROR, "wpas_mesh_peer_remove fail!");
			goto OUT;
		}
...
static int wpa_mesh_set_accept(struct wpa_supplicant *wpa_s, const char *buf)
{
	enum hisi_mesh_enable_flag_type flag = HISI_MESH_ENABLE_FLAG_BUTT;
	unsigned char enable_connect;
	(void)wpa_s;
	if (os_strncmp(buf, "STA ", 4) == 0) {
		flag = HISI_MESH_ENABLE_ACCEPT_STA;
		enable_connect = !!atoi(buf + 4);
	} else if (os_strncmp(buf, "PEER ", 5) == 0) {
		flag = HISI_MESH_ENABLE_ACCEPT_PEER;
		enable_connect = !!atoi(buf + 5);
	} else {
		wpa_error_log0(MSG_ERROR, "wpa_mesh_set_accept invalid input.");
		return HISI_FAIL;
	}
	return wifi_mesh_set_accept(enable_connect, flag);
}
#endif /* LOS_CONFIG_MESH */
...
static int wpa_supplicant_show_sta(hi_wifi_iftype iftype)
{
	...
	for (sta = g_hapd->sta_list; sta != NULL; sta = sta->next) {
		...
		if ((iftype == HI_WIFI_IFTYPE_AP) && (sta->flags & WLAN_STA_AUTHORIZED)) {
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strncmp(buf, "AP_DEAUTH ", 10) == 0) {
		if (wifi_dev->iftype == HI_WIFI_IFTYPE_AP)
			(void)hapd_deauth_peer((struct hostapd_data *)wpa_s, buf + 10);
		goto OUT;
	...
	if ((os_strncmp(buf, "STA_SET_DELAY_RP ", 17) == 0) && (wifi_dev->iftype == HI_WIFI_IFTYPE_STATION)) {
		if (wpa_drv_set_delay_report(wpa_s, atoi(buf + 17)) != HISI_OK)
			wpa_error_log1(MSG_ERROR, "STA_SET_DELAY_RP set_delay_report %d fail!", atoi(buf + 17));
		goto OUT;
	}
	...
	if (os_strncmp(buf, "STATUS", 6) == 0) {
		...
		if ((g_sta_delay_report_flag == WPA_FLAG_ON) && (wpa_drv_set_delay_report(wpa_s, WPA_FLAG_OFF) != HISI_OK))
			wpa_error_log0(MSG_ERROR, "ctrl iface: wpa_drv_set_delay_report off fail!");
