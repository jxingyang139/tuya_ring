...
#endif /* HISI_CODE_CROP */
	if (state == WPA_COMPLETED) {
		if ((g_reconnect_set.enable == WPA_FLAG_ON) && (wpa_drv_set_delay_report(wpa_s, WPA_FLAG_ON) != HISI_OK))
			wpa_error_log0(MSG_ERROR, "wpa_drv_set_delay_report on fail!");
...
int wpa_supplicant_remove_network(struct wpa_supplicant *wpa_s, int id)
{
	...
	if (ssid->mode == WPAS_MODE_INFRA) {
		...
		if ((g_sta_delay_report_flag == WPA_FLAG_ON) && (wpa_drv_set_delay_report(wpa_s, WPA_FLAG_OFF) != HISI_OK))
			wpa_error_log0(MSG_ERROR, "wpa_drv_set_delay_report off fail!");
		g_sta_delay_report_flag = WPA_FLAG_OFF;
	}
#endif /* LOS_WPA_PATCH */
#ifndef LOS_CONFIG_NO_PMKSA
