...
/* 0: not upgraded; 1: upgrade the kernel version; 2: Upgrade the boot version. */
...
{
    if (update_ver_type != UPG_UPDATE_VER_FIRMARE && update_ver_type != UPG_UPDATE_VER_BOOT) {
...
hi_u32 upg_get_decrpt_rsa_key(HI_CONST boot_header *head, hi_u8 *key, hi_u32 key_len)
{
    ...
    if (memcpy_s(key, key_len, raw_data + (ctx->rsa_key_addr - head->code_section_offset), RSA_2048_LEN) != EOK) {
        ret = HI_ERR_FAILURE;
        goto end;
    }
    key[key_len - 1] = 0x01; /* 1: first byte. 0x01: part of key_e 0x10001 */
...
hi_u32 upg_get_nv_cfg(hi_nv_ftm_startup_cfg *startup_cfg, hi_nv_ftm_upg_wait_mode *mode_cfg)
{
    hi_u32 ret = upg_get_start_up_cfg(startup_cfg);
    if (ret != HI_ERR_SUCCESS) {
        upg_stop(ret);
        return ret;
    }
...
hi_u32 hi_upg_finish_with_cache(hi_void)
{
    ...
    hi_nv_ftm_upg_wait_mode mode_cfg = {0};
    upg_ctx *ctx = upg_get_ctx();
    hi_u32 ret = upg_get_nv_cfg(&cfg, &mode_cfg);
    if (ret != HI_ERR_SUCCESS) {
        return ret;
    }
    ...
    if (ctx->common_head.file_type == HI_UPG_FILE_BOOT) {
        ...
        /* Change kernel start address. */
#ifndef CONFIG_COMPRESSION_OTA_SUPPORT
        cfg.addr_start = ctx->file_addr;
#endif
        cfg.addr_write = ctx->file_addr;
        cfg.refresh_nv = ENV_REFRESH_NV;
